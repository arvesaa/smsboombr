...
TOKEN = "MTEyMTIwMDgyODQ2ODk1NzI3NA.Gc2SIs.K_3tu0xADS9kDvZhFx-ikvNES8MOYoeIcYwTLg"
...
async def on_ready():
    ...
    activity = discord.Activity(type=discord.ActivityType.playing, name="Freak Baş Edemezsin")
...
async def on_message(message):
    ...
    if len(message.content.split(" ")) == 2 and message.content.split(" ")[0] == ".sms":
        if len(message.content.split(" ")[1]) == 10:
            ...
            embed=discord.Embed(title="Freak Boomber (+90)", description=(f"{adet} adet SMS Gönderiliyor -->\n{message.author.mention}"), color=0x001eff)
